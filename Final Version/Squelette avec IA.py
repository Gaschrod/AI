...
def terminal_display(map_size, total_turn, no_damage_turn, werewolf_dico, food_dico):
    ...
    implementation : Gautier Hisette (v.4 28/04/2022)
...
def no_obstacle(x_position_origin, y_position_origin, x_position_target, y_position_target, werewolf_dico):
    """
    Verify that the displacement is possible and there are no obstacle
    ...
    Returns :
    ---------
    order : order for the displacement or X if there is an obstacle (str)

    Versions :
    ----------
    specification : Alexis Regardin (v.1 28/04/2022)
    implementation : Alexis Regardin (v.1 28/04/2022)
    ...
    if target not in werewolf_dico :
        order = origin + ':@' + target
        return order
    else:
        order = 'X'
...
def AI(werewolf_dico, food_dico, team):
    ...
    for wolves in AI_wolves:
        ...
        for enemy in enemy_wolves:
            ...
            if werewolf_dico[enemy][2] > 0 and werewolf_dico[wolves][2] > 10: #If the wolf has enough energy left to fight
            ...
            if werewolf_dico[enemy][1] == 'alpha' and werewolf_dico[wolves][2] > 10: #If the wolf has enough energy left to fight
                if total_difference in [-1,0,1] and (x_position_target - x_position_origin) in [-1,0,1] and (y_position_target - y_position_origin) in [-1,0,1]: #Check if the target is 0 or 1 square away from the origin 
                        order_turn = AI_attack(x_position_origin, y_position_origin, x_position_target, y_position_target) #Create an attack order like this : x-y:*x-y 
                        orders += (order_turn + ' ') #Add the order        
                else: #The wolf isn't next to the alpha  
                    if x_difference < -1:
                        x_position_moving = x_position_origin - 1
                        legal_move = no_obstacle(x_position_origin, y_position_origin, x_position_moving, y_position_origin, werewolf_dico)
                        if legal_move != 'X':
                            order_turn = legal_move
                        else:
                            x_position_moving = x_position_origin + 1
                            order_turn = no_obstacle(x_position_origin, y_position_origin, x_position_moving , y_position_origin, werewolf_dico)

                    elif x_difference > 1:
                        x_position_moving = x_position_origin + 1
                        legal_move = no_obstacle(x_position_origin, y_position_origin, x_position_moving, y_position_origin, werewolf_dico)
                        if legal_move != 'X': #There is no obstacle
                            order_turn = legal_move
                        else:
                            x_position_moving = x_position_origin - 1
                            order_turn = no_obstacle(x_position_origin, y_position_origin, x_position_moving , y_position_origin, werewolf_dico)
                    ...
                    if y_difference < -1:
                        ...
                        legal_move = no_obstacle(x_position_origin, y_position_origin, x_position_origin, y_position_moving, werewolf_dico)
                        if legal_move != 'X': #There is no obstacle
                            order_turn = legal_move
                        else:
                            y_position_moving = y_position_origin + 1
                            order_turn = no_obstacle(x_position_origin, y_position_origin, x_position_origin, y_position_moving, werewolf_dico)
...
def play_game(map_path, group_1, type_1, group_2, type_2):
    ...
    if type_1 == 'Remote' or type_2 == 'Remote':
        connection = create_connection(11,12)
